...
float CNN::preprocess_and_predict(ClusterNetId net_id, t_bb* bbptr, unsigned long number_of_terminals,
    ...
    uint8_t image[1][CNN_INPUT_GRID_SIZE][CNN_INPUT_GRID_SIZE] = {0}; // initialize whole array to 0
    ...
    image[0][x - bbptr->xmin][y - bbptr->ymin] = 1; // set pixel at position of source terminal to 1
    ...
    for(auto pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
        ...
        image[0][x - bbptr->xmin][y - bbptr->ymin] = 1; // set pixel at position of current sink terminal to 1
    }
...
float CNN::encode_and_predict(void* mapped_input, unsigned long terminal_count) {
    ...
    int ndims = 3;
    int64_t dims[] = {1, CNN_INPUT_GRID_SIZE, CNN_INPUT_GRID_SIZE};
    for(int i = 0; i < CNN_INPUT_GRID_SIZE; i++){
        for(int j = 0; j < CNN_INPUT_GRID_SIZE; j++) {
            printf("%d ", ((uint8_t (*)[CNN_INPUT_GRID_SIZE][CNN_INPUT_GRID_SIZE]) mapped_input)[0][i][j]);
        }
        printf("\n");
    }
    ...
    TF_Tensor *float_tensor = TF_NewTensor(TF_FLOAT, dims, ndims,
            ((uint8_t (*)[CNN_INPUT_GRID_SIZE][CNN_INPUT_GRID_SIZE]) mapped_input), ndata, &NoOpDeallocator,
            0);
