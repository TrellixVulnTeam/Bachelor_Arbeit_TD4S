...
for index in range(len(data_train)):
    input_data_set = data_train[index][2]
...
for data in data_validate:
    while len(individual_loss_histories) < (len(data[2]) - 1):
        individual_loss_histories.append([[], 0, 0])
    individual_loss_histories[len(data[2]) - 2][1] += 1
...
for epoch_count in range(epochs):
    ...
    for batch_counter in range(len(list_of_batches)):
        ...
        x_train = np.asarray([data[2] for data in list_of_batches[batch_counter]])
        x_train = x_train.reshape(batch_size, len(list_of_batches[batch_counter][0][2]), 2)
        y_train = np.asarray([data[3] for data in list_of_batches[batch_counter]])
    ...
    for data in data_validate:
        result = model_for_validation.predict(np.asarray(data[2]).reshape(1, len(data[2]), 2))
        ...
        if loss_function == 'mean_squared_error':
            squared_error = (result - data[3]) * (result - data[3])
            if squared_error > 5:
                print("large error detected:", squared_error)
                print("expected value:", data[3])
                print("computed value:", result)
                print("sequence length:", len(data[2]))
                print("sequence:", data[2])
            squared_loss_sum += squared_error
            individual_loss_histories[len(data[2]) - 2][0][epoch_count] += squared_error
        else:
    ...
    if(epoch_count == 0):
        for data in data_validate:
            if loss_function == 'mean_squared_error':
                squared_error = (data[1] - data[3]) * (data[1] - data[3])
                squared_original_bb_loss_sum += squared_error
                individual_loss_histories[len(data[2]) - 2][2] += squared_error
            else:
                print("ERROR: loss function not implemented")
                sys.exit("failed to compute loss function: not implemented.")
    ...
    if (epoch_count == 0) | (epoch_count == epochs - 1) | (epoch_count == int(epochs / 2)):
        ...
        plt.scatter(range(validation_plotting_point_count),
                    ...
                    c='r')
        plt.scatter(range(validation_plotting_point_count),
                    [data[3] for data in data_validate][:validation_plotting_point_count],
                    c='g')
        plt.scatter(range(validation_plotting_point_count),
                    [data[1] for data in data_validate][:validation_plotting_point_count],
                    c='b')
    ...
    plt.axhline(y=squared_original_bb_loss_sum, color='g', linestyle='-')
